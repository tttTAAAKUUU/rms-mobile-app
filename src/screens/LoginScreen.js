...
const handleLogIn = async ()=>{
    if (email && password) {
        try {
            ...
            navigation.navigate('Home', { em: email })
            
        } catch (error) {
            alert('Enter valid email password combonation')
        }
    }
}
...
return(
    <SafeAreaView style={{flex:1, justifyContent:'center'}}>
        <View style={{paddingHorizontal:25}}>
            ...
            <View style={{flexDirection:'row', justifyContent:'centers'}}>
                <Text style={{ fontWeight:'700'}}>Are you a specialist?</Text>
                <TouchableOpacity onPress={()=>{navigation.navigate('speclog')}}><Text style={{color:'#AD40AF', fontWeight:'700'}}> login here</Text></TouchableOpacity>
            </View>
